...
- 2026-09-10: **MAPIT-tabin PELAA-nappi ei näkynyt** (`SurvivorProceduralMaps.cs`, `DrawMapTypeTab()`) - lista (otsikko + SATUNNAINEN + 3 karttatyyppiä x 3 Tieriä + PELAA-nappi) piirtyi yhteensä ~800px korkeuteen 720px-korkealla canvasilla, joten nappi jäi näkyvän alueen ulkopuolelle eikä sitä voinut klikata. Korjattu samalla `GUI.BeginScrollView`/`EndScrollView`-ratkaisulla kuin HAHMOT-tabissa (`CharacterShop.cs`, `characterTabScroll`) - uusi `mapTypeTabScroll`-kenttä, viewport 1280x720, content 1260x820. Kaikki tabin sisäiset absoluuttikoordinaatit muuttumattomia.
