...
- 2026-09-10: **MAPIT-tabin PELAA-nappi ei näkynyt** (`SurvivorProceduralMaps.cs`, `DrawMapTypeTab()`) - lista (otsikko + SATUNNAINEN + 3 karttatyyppiä x 3 Tieriä + PELAA-nappi) piirtyi yhteensä ~800px korkeuteen 720px-korkealla canvasilla, joten nappi jäi näkyvän alueen ulkopuolelle eikä sitä voinut klikata. Ensimmäinen yritys lisäsi `GUI.BeginScrollView`/`EndScrollView` (sama ratkaisu kuin HAHMOT-tabissa) - hylättiin heti pelaajapalautteen perusteella, koska IMGUI-scrollbar oli liian huomaamaton eikä pelaaja löytänyt sitä. Lopullinen korjaus: tiivistettiin rivikorkeudet (Procedural-rivin jälkeinen väli 56→44, karttarivien väli 46→36, rivikorkeus 40→32) niin että koko lista mahtuu kokonaan 720px:n sisään ilman vierittämistä (PELAA-nappi päättyy nyt ~y674:ään). Julkaistu patch-releasena v0.4.1.

- 2026-09-10: **Julkaisu v0.4.0** - build StandaloneWindows64:lle Unity CLI:n kautta (0 build-virhettä, 16 varoitusta, 302 MB, ~99 s), pakattu `Builds/SurvivorPrototype-v0.4.0.zip`, julkaistu GitHub Releasena (https://github.com/Marcebooy/SurvivorPrototype/releases/tag/v0.4.0). Sisältää kaikki v0.3.5:n jälkeiset commitit: WeaponAttachmentPoint-systeemi + 29 asetta yhdistetty, Tuhkaerämaa-kartta, Mappi-tilan viholliskiintiö Tierin mukaan, ase-VFX-poolisysteemi, Necromancer-bossin uusiksikirjoitus, Inferno-areena, koko WeaponArtPilot-assettikirjasto. **Ei Discord-ilmoitusta** käyttäjän nimenomaisesta pyynnöstä.

- 2026-09-10: **Mappi-tilan viholliskiintiö Tierin mukaan, aikasidonnaisen 90s-spawnin sijaan** (pelaaja-progressio-ja-ranked-suunnitelma.md, kohta 3) - koskee kaikkia kolmea kiinteää karttaa (Mosswood/Luuluola/Tuhkaerämaa) yhtä lailla, koska ne kaikki kulkevat saman `SurvivorGame.cs`/`SurvivorProgression.cs`-ydinsilmukan läpi eikä mikään tästä eteenpäin haaraudu `MapType`:n mukaan - vain `activeMapType != Procedural`. Selviytymistila (Procedural) täysin muuttumaton.
  - **Uusi `TierEnemyQuota(int tier)`** (`SurvivorProceduralMaps.cs`, `MapTierMultiplier`:n vierestä): T1=150, T2=220, T3=300 - kokonaisspawnimäärä yhdellä alueella, ei samanaikaisten yläraja (se on edelleen erillinen `enemyCap`).
  - **Uudet jaetut apuominaisuudet** `AreaSpawningStopped` ja `AreaBossReady`: Selviytymistilassa molemmat identtiset alkuperäiseen 90s-ajastimeen (`Elapsed - areaStarted >= 90`, ei koskaan lue `areaKillCount`:ia). Mappi-tilassa `AreaSpawningStopped` = `areaKillCount >= TierEnemyQuota(activeMapTier)`, ja `AreaBossReady` vaatii LISÄKSI `enemies.Count == 0` (Selviytymistilassa bossin voi kutsua vaikka kentällä olisi vielä vihollisia, kuten ennenkin - tätä ei muutettu).
  - **Uusi `areaKillCount`-kenttä** (`SurvivorProgression.cs`, `Kills`:stä erillinen - `Kills` ei nollaudu koskaan kesken runin, `areaKillCount` nollataan `ResetProgression()`:issa ja `AdvanceArea()`:ssa samalla tavalla kuin `areaStarted`). Kasvatetaan `Hit()`:issä `Kills++`:n vierestä, mutta EI bossin kohdalla (`wasBoss`-lippu talteen ennen `boss = null`-mutaatiota, koska tarkistus `e == boss` epäonnistuisi hiljaisesti jos se tehtäisiin vasta sen jälkeen - löytyi ja korjattiin ensimmäisellä kirjoituskerralla ennen testausta).
  - **`SurvivorGame.cs`:n `Update()`**: vanha `Elapsed - areaStarted < 90` korvattu `!AreaSpawningStopped`:illa - Selviytymistilassa sama käytös, Mappi-tilassa spawni jatkuu ajasta riippumatta kunnes kiintiö täyttyy. `enemyCap`-vertailu koskematon.
  - **`SurvivorProgression.cs`**: `TickProgression()`:in "Uusia vihollisia ei enää tule" -ilmoitus, `Interact()`:in bossinkutsuehto ja `LandmarkHint()`/uusi jaettu `AreaPortalLabel()`-apumetodi (poistaa koodin toiston `LandmarkHint`:in ja `DrawProgression`:in HUD-tekstin väliltä) käyttävät nyt `AreaSpawningStopped`/`AreaBossReady`:ia suoran `Elapsed - areaStarted`-vertailun sijaan. UI näyttää Mappi-tilassa "X/Y vihollista tapettu" 90s-ajastimen sijaan samassa kohdassa (interactionHint-rivi ja oikean yläkulman HUD-teksti).
  - Vihollisten HP/nopeus-skaalauskaava (`SpawnEnemy()`, Elapsed-pohjainen) ja bossitaistelu itsessään koskemattomia - vain spawnin määrä/lopetusehto muuttui, ei vihollisten vahvuus.
  - **Testattu Unity CLI:llä Play Modessa** (reflektiolla, koska 150 oikean vihollisen tappaminen olisi liian hidasta ajaa Play Modessa): Mosswood T1 - `TierEnemyQuota(1)`=150 vahvistettu; `areaKillCount`=149→`AreaSpawningStopped`=false, =150→true (tarkka raja täsmää); `AreaBossReady`=false niin kauan kuin `enemies.Count>0` vaikka kiintiö täynnä; vihollislistan tyhjennyksen jälkeen `AreaBossReady`=true ja oikea `Interact()`-kutsu (synteettinen bossi-maamerkki, kind=2) tuotti todellisen bossin (`boss` ei-null) - täsmälleen samalla mekanismilla kuin ennen. Selviytymistilassa (Procedural) `areaKillCount` pakotettu 999999:ään ja `AreaSpawningStopped`/`AreaBossReady` pysyivät molemmat false:na tuoreella alueella (0-vaikutus vahvistettu) - 91s:n simuloinnilla molemmat kääntyivät true:ksi täysin alkuperäisen kaavan mukaisesti, myös `enemies.Count>0`:lla (Proceduralissa ei koskaan vaadittu tyhjää kenttää, kuten ennenkään). `AreaPortalLabel()` palautti oikean tekstin kaikissa kolmessa tilassa ("Portaali avautuu: 90 s" / "42/150 vihollista tapettu" / "150/150 vihollista tapettu"). 0 uutta konsolivirhettä koko testin ajan (ainoat virheet olivat jo aiemmin dokumentoitu NetworkObject-siivousjäänne testin omista `Object.Destroy`-kutsuista vihollisten NetworkObject-komponenteille, ei liity tähän muutokseen). Käännös 0 virhettä.
