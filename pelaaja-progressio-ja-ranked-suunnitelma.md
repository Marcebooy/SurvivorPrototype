...
**10.3. Ase-affiksien craftausjärjestelmä — TOTEUTETTU, 19/30 aseella (ehdotettu ja hyväksytty 2026-09-09, committoitu 2026-09-09 kolmessa erässä).**

**Jäljitys — mitkä 19/30 asetta on affiksitettu (uusin ylimpänä):**
| Erä | Asetta | Aseet | applicableStats-lähde |
|---|---|---|---|
| 3 (kohta 10.5:n jälkeen) | 6 | Mines, Tornado, Poison Flask, Space Noodle, Slutty Rocket, Wireless Dagger | `WeaponStats.cs`:n `WeaponMetadata`, luettu automaattisesti (ei käsin kirjoitettu) |
...
**Tila**: **Committoitu kolmessa erässä** — pilotti `44ee574`+`1fc7f1b`, laajennus `1669638`+`146c923`, erä 3 (Mines/Tornado/PoisonFlask/SpaceNoodle/Rocket/WirelessDagger), kaikki 2026-09-09. Työhakemistossa oli useilla kerroilla samaan aikaan toisen, tähän liittymättömän session WIP (VFX-poolisysteemi, Necromancer-bossi, Tuhkaerämaa-taidepaketti, Art_Prototype_Stylized/Feyloom-assetit) samoissa jaetuissa tiedostoissa — jokaisessa erässä Code erotti muutokset täsmällisesti diffillä (HEAD+omat-muutokset-menettely, varmennettu diffillä molempiin suuntiin), committasi vain affiksit, ja palautti muun WIP:n koskemattomana takaisin. Erässä 3 ei koskettu SurvivorArsenal.cs/SurvivorGame.cs/SurvivorProgression.cs/SurvivorAdvancedWeapons.cs:ään, Assets/WeaponArtPilot/-, WeaponBases/-hakemistoihin eikä WeaponAttachmentPoint.cs/SwordVariantVisual.cs-tiedostoihin - vain `WeaponAffixes.cs` sekä tämän dokumentin ja AGENTS.md:n affiksia koskevat kohdat.
